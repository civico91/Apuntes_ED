...
- Pruebas unitarias (o de unidad) -> Pruebas iniciales donde se prueban las clases y los métodos. Son pruebas de unidad.
- Pruebas de regresión -> Cuando vamos escribiendo código nuevo. Comprueba que al meter nuevo código, no se introducen errores que antes no existian.
- Pruebas de integración -> Unir todo y que funcione correctamente en conjunto. Son menos automatizables.
- Pruebas del sistema -> Son las pruebas no funcionales. De rendimiento, seguridad...
- Pruebas de humo (smoke test) -> Comprueba los componentes o funcionalidad básica de la aplicación,(login, conexión a la base de datos..).
- Pruebas alfa y beta -> Las alfa son pruebas que se realizan dentro de la empresa, las beta se hacen en un entorno no controlado.
